...
export default function GameGrid({ items, imageById = {}, fallbackImages = [] }) {
  const handleDragStart = (e, gameId, imgSrc) => {
    try {
      e.dataTransfer.setData("application/json", JSON.stringify({ gameId, quantity: 1 }));
    ...
    <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {items.map((p, idx) => (
        <div
          key={p.id}
          draggable
          onDragStart={(e) => handleDragStart(e, p.id, imageById[p.id] || fallbackImages[idx % fallbackImages.length] || fallbackImages[0])}
        ...
        >
          ...
          <div className="relative w-full h-48 bg-gray-100">
            <Image
              src={imageById[p.id] || fallbackImages[idx % fallbackImages.length] || fallbackImages[0]}
          ...
          <div className="p-5 flex flex-col flex-1">
            ...
            <div className="mt-auto pt-4 flex items-center justify-between gap-3">
              <a
                href={`/games/${p.id}`}
                className="flex-1 text-center rounded-full px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 border border-gray-300"
              >
                View Details
              </a>
